...
@section('heading')
   <div class="pt-4 pb-2">
      ...
      <div class="d-flex justify-content-between align-items-center w-100 flex-wrap">
         <h1 class="h4 mb-3 mb-lg-0">Data Absensi</h1>
         <div class="btn-group" role="group">
            <button id="create" type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown"
               aria-expanded="false">
               &plus; Absensi
            </button>
            <ul class="dropdown-menu" aria-labelledby="create">
               <li><a class="dropdown-item" href="/dashboard/absensi?create=rayon">Rayon</a></li>
               <li><a class="dropdown-item" href="/dashboard/absensi?create=rombel">Rombel</a></li>
            </ul>
         </div>
      </div>
   </div>
                  ...
                  @if (count($data))
                     @foreach ($data as $row)
                        <tr>
                           ...
                           @if (Auth::user()->role == 'Admin' || Auth::id() == $row->user->id)
                              <td>
                                 <a href="/dashboard/absensi/{{ $row->id }}/siswa" class="btn btn-info"
                                    href="#">Detail</a>
                                 <a href="/dashboard/absensi/{{ $row->id }}/edit" class="btn btn-warning"
                                    href="#">Edit</a>
               ...
               fetch('http://lara-absensi.test/dashboard/absensi/' + id, {
                  ...
                  .then(data => {
                     document.querySelector('#modal-notification p').innerHTML =
                        `Apakah kamu yakin ingin menghapus absensi ini? Semua data yang bersangkutan dengan absensi <b>${data.name}</b> juga ini akan ikut terhapus!`;
                     document.querySelector('#form-delete').setAttribute('action',
                        '/dashboard/absensi/' + data.id + '/delete');
