...
@section('title', 'Tambah Data Siswa | Abseen')
   ...
   <div class="pt-4 pb-2">
      <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
         <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
            ...
            <li class="breadcrumb-item active"><a href="/dashboard/user/siswa/create">Tambah Data Siswa</a></li>
         </ol>
      </nav>
      <div class="d-flex justify-content-between align-items-center w-100 flex-wrap">
         <h1 class="h4 mb-3 mb-lg-0">Tambah Data Siswa</h1>
...
@section('content')
   <div class="card border-0 shadow mb-4">
      ...
      <div class="card-body">
         <form action="/dashboard/user/siswa/create" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
               <div class="col-sm-6">
                  <div class="mb-4">
               ...
               <div class="col-sm-6">
                  <div class="mb-4">
                     ...
                     <input type="number" class="form-control @error('nis') is-invalid @enderror" name="nis" id="nis"
                        required autocomplete="off" value="{{ old('nis') }}">
                     @error('nis')
                        <div class="invalid-feedback">
                           {{ $message }}
                        </div>
                  ...
                  <div class="mb-4">
                     ...
                     <select class="form-select @error('rombel') is-invalid @enderror" name="rombel" id="rombel" required
                        ...
                        @foreach ($rombel as $row)
                           <option {{ $row->id == old('rombel') ? 'selected' : '' }} value="{{ $row->id }}">
                              {{ $row->name }}</option>
                        @endforeach
                     </select>
                     @error('rombel')
                  ...
                  <div class="mb-4">
                     ...
                     <select class="form-select @error('rayon') is-invalid @enderror" name="rayon" id="rayon" required
                        ...
                        @foreach ($rayon as $row)
                           <option {{ $row->id == old('rayon') ? 'selected' : '' }} value="{{ $row->id }}">
                              {{ $row->name }}</option>
                        @endforeach
                     </select>
                     @error('rayon')
               ...
               <div class="col-sm-6">
                  <div class="mb-4">
                     <label for="password_confirmation">Konfirmasi Password Siswa:</label>
